// IncomeRepository: MongoDB-backed CRUD with localStorage fallback
...
(function() {
  ...
  async function getAll() {
    ...
    try {
      ...
      console.warn('⚠️  MongoDB unavailable for income, using localStorage:', error.message);
      useMongoDb = false;
      return getLocalAll();
    }
  }
  ...
  async function add(income) {
    ...
    try {
      ...
      console.warn('⚠️  MongoDB add failed for income, using localStorage only:', error.message);
      useMongoDb = false;
      const localList = getLocalAll();
      localList.push(income);
      saveLocalAll(localList);
  ...
  async function update(updated) {
    ...
    try {
      ...
      console.warn('⚠️  MongoDB update failed for income, using localStorage only:', error.message);
      useMongoDb = false;
      const localList = getLocalAll().map(e => e.id === updated.id ? { ...e, ...updated } : e);
      saveLocalAll(localList);
      return localList;
  ...
  async function removeById(id) {
    ...
    try {
      ...
      console.warn('⚠️  MongoDB delete failed for income, using localStorage only:', error.message);
      useMongoDb = false;
      const localList = getLocalAll().filter(e => e.id !== id);
      saveLocalAll(localList);
      return localList;
  ...
  checkMongoDb().then(async (available) => {
    ...
    if (available) {
      console.log('✅ IncomeRepository: MongoDB connected');
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (mongoData.length === 0 && localData.length > 0) {
            console.log('🔄 Syncing localStorage income to MongoDB...', localData.length, 'items');
            ...
            if (bulkResponse.ok) {
              console.log('✅ Income synced to MongoDB');
            }
          } else if (mongoData.length > 0) {
            // MongoDB has data, sync to localStorage
            saveLocalAll(mongoData);
            console.log('✅ Income synced from MongoDB:', mongoData.length, 'items');
          }
        }
      } catch (err) {
        console.warn('⚠️ Could not sync income:', err.message);
      }
    } else {
      console.log('⚠️  IncomeRepository: Using localStorage only');
    }
  });

  window.App.Repositories.IncomeRepository = {
    getAll,
    add,
    update,
    removeById
